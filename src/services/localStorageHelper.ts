...
export function getNewestBooksWithAuthor(
  count: number = 5,
): BookWithAuthorName[] | null {
  const books = getBooks();
...
export function getBookWithAuthorById(
  bookId: number,
): BookWithAuthorName | null {
  const books = getBooks();
  const authors = getAuthors();
...
export function getCurrentUser() {
  ...
  const users = getFromLocalStorage<User[]>("users");
  if (users) {
    return users.find((user) => user.token === currentUserToken) ?? null;
  }
  return null;
}
export function getCurrentCart() {
  return getFromLocalStorage<CartItemType[]>("cart");
}

export function checkToken(): boolean {
  let checkResult = false;
  const savedToken = getCurrentUserToken();
  const users = getFromLocalStorage<User[]>("users");
  users?.forEach((user) => {
